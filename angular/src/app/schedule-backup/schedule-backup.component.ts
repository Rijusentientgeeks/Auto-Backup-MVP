...
import { SourceConfiguationDto, SourceConfiguationServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class ScheduleBackupComponent  implements OnInit{
  ...
  generateCron(frequency: string, dayOfWeek: number | string, time: string, dayOfMonth: number | string): string {
    ...
    switch (frequency) {
      ...
      case 'monthly':
        return `${minute} ${hour} ${dayOfMonth !== null ? dayOfMonth : '*' } * *`; // Use dayOfMonth if available
      default:
        return '';
    }
  }
...
debugger
  ...
  scheduleTask(cronExpression: string): void {
    ...
    // Add API call here to send cronExpression to backend
    // this.http.post('/api/schedule-task', { cron: cronExpression }).subscribe();
  }
}
